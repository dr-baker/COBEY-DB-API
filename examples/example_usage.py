...
class WorkoutSessionClient:
    ...
    def save_workout_session(self, userid: str, session_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def query_sessions(self) -> Dict[str, Any]:
        ...
        response = self.client.get(f"{self.base_url}/api/sessions/")
        response.raise_for_status()
        return response.json()
...
def main():
    ...
    try:
        ...
        session_response = client.save_workout_session("test_user", workout_data)
        print(f"Save Session Response: {session_response}")
